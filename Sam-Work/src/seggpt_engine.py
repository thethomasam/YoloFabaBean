...
def run_one_image(img, tgt, model, device):
    ...
    mask2, y, mask = model(x.float().to(device), tgt.float().to(device), bool_masked_pos.to(
        device), valid.float().to(device), seg_type.to(device), feat_ensemble)
...
def inference_image(model, device, img_path, img2_path, tgt2_path, out_path, mask_out_path):
    res, hres = 448, 448
    ...
    output = Image.fromarray(
        (input_image * (1 * output / 255 + 0.9)).astype(np.uint8))
...
def inference_video(model, device, vid_path, num_frames, img2_paths, tgt2_paths, out_path):
    ...
    while True:
        ...
        video_writer.write(np.ascontiguousarray(
            output.astype(np.uint8)[:, :, ::-1]))
